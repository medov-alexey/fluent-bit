...
if [ "$1" == "delete" ] || [ "$1" == "drop" ] || [ "$1" == "uninstall" ] || [ "$1" == "remove" ]; then
   docker rm -f fluent-bit mongo elasticsearch graylog testlog testlogsmall> /dev/null 2>&1
...
docker run -d --name testlog -v $(pwd)/logs_for_demo:/tmp/ --log-opt tag=docker-container-tag --log-opt max-size=2MB --log-opt max-file=3 python:3.10-alpine python /tmp/print.py

docker run -d --name testlogsmall -v $(pwd)/logs_for_demo:/tmp/ --log-opt tag=docker-container-tag --log-opt max-size=2MB --log-opt max-file=3 python:3.10-alpine python /tmp/print_small.py
